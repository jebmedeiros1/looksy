...
import { Sparkles, ShirtIcon, Wand2, ArrowRight, Heart, Zap, Star, Store, X, Quote } from "lucide-react";
import { MOTIVATIONAL_MESSAGES } from "@/lib/types";

function MotivationalModal({ onClose }: { onClose: () => void }) {
  const msg = MOTIVATIONAL_MESSAGES[Math.floor(Math.random() * MOTIVATIONAL_MESSAGES.length)];
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm animate-fade-in">
      <div className="bg-white rounded-3xl shadow-2xl w-full max-w-md overflow-hidden">
        <div className="bg-gradient-to-br from-brand-700 to-accent-500 p-8 text-white relative">
          <button
            onClick={onClose}
          ...
          <p className="text-lg font-semibold leading-relaxed">{msg.message}</p>
          <p className="text-white/70 text-sm mt-4 font-medium">— {msg.author}</p>
        </div>
        <div className="px-8 py-5 flex items-center justify-between">
          <span className="text-xs font-semibold text-brand-600 bg-brand-50 px-3 py-1.5 rounded-full">
            {msg.archetype}
          </span>
          <button
            onClick={onClose}
...
export default function Home() {
  ...
  useEffect(() => {
    ...
    // Show motivational modal once per session
    const shown = sessionStorage.getItem("motivation_shown");
    if (!shown) {
      setShowMotivation(true);
      sessionStorage.setItem("motivation_shown", "1");
    }
  }, []);
      ...
      <div className="max-w-5xl mx-auto px-4 py-12">
        ...
        <div className="grid md:grid-cols-4 gap-5 mb-16">
          {[
            ...
            { icon: Wand2, color: "bg-purple-100 text-purple-700", title: "Receba 3 composições únicas", desc: "Cada look pensa no seu corpo, seu contexto e a narrativa que você quer contar." },
            { icon: Store, color: "bg-teal-100 text-teal-700", title: "Loja de Estilos", desc: "Explore guarda-roupas de outras usuárias e adicione peças ao seu acervo." },
        ...
        {/* Motivational re-trigger */}
        <div className="flex items-center justify-center mb-8">
          <button
            onClick={() => setShowMotivation(true)}
            className="flex items-center gap-2 text-sm text-brand-600 hover:text-brand-800 font-medium transition"
          >
            <Quote className="w-4 h-4" /> Receber uma mensagem do dia
          </button>
        </div>
